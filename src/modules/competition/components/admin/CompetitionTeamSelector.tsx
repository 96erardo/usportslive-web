...
function CompetitionTeamSelector (props: Props) {
  const { isOpen, args, closeModal, openModal } = useModal(`${props.id}-competition-team-selector`);
  ...
  const close = useCallback(() => closeModal(`${props.id}-competition-team-selector`), [closeModal, props.id]);
  
  const open = useCallback((args: DialogArgs) => openModal(`${props.id}-competition-team-selector`, args), [openModal, props.id]);
