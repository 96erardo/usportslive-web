 ...
 */
export function usePersons (page: number = 1, include: Array<string> = [], initialFilters = {}) {
  ...
  const fetch = useCallback(async () => {
    ...
    const [error, canceled, data] = await fetchPersons(page, include, filters, cancelToken.current);
